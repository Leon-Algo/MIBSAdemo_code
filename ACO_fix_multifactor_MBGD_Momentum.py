...
class ACO():
    def __init__(self,map_data,route_load,node_load,start = [0,0],end = [19,19],tao_max = 10,tao_min=0.0001,ant_gener_pher =8,init_pher_pow=6,max_iter = 100,ant_num = 50,pher_imp = 1,dis_imp =10,evaporate_init = 0.9,) -> None:
        '''
            Params:
            --------
                pher_imp : 信息素重要性系数
        ...
        self.ant_gener_pher =ant_gener_pher     #每只蚂蚁携带的最大信息素总量
...
##############################################333
        ...
        self.route_load = route_load.copy()  
        pher_data_init = np.zeros(shape=[self.map_lenght**2,self.map_lenght**2]) + init_pher_pow # 初始信息素全部设为6
        j_2 = np.array(self.route_load)
        j_3 = j_2[:,0]*self.map_lenght+j_2[:,1]
...
################################################################？？？？
        ...
        self.generation_DIV = []  #每代的路径搜索的多样性值--变异程度(大小)，绘迭代图用   DIV = 样本标准差
    ...
    def run(self):
        ...
        #     print(f'a1={a1}, a2={a2}, a3={a3}')
            ...
            for i in range(self.max_iter):      
                ...
                #Step 2:计算每条路径对应的长度，经过的节点累积载荷，经过的站点数，后用于信息素的生成量，以及多因素权重系数
                way_lenght_list = []
                ...
                #########################所以相当于把栅格地图onehot展开，用前后格子的序号来构成二维坐标(表示前进路径方向上的信息素)
                    ...
                    self.cost.append((a1*way_lenght_list[k] + a2*success_way_node_load_sum[k] + a3*zhandian_count[k]))   #########这里的cost是全局的(至今的所有cost)
    ...
    #                                            #################################多因素：全路径长度、全路径节点载荷、路径节点个数
                ...
                # self.generation_DIV.append(statistics.stdev(way_lenght_list))
                ...
                #     self.ways_data.append(copy.deepcopy(success_way_list[a_1]))######储存四组不同权重系数组合的路线的列表



                # if self.way_zuhe_best[zuhe]>=min(way_lenght_list):
                #     a_1 = way_lenght_list.index(min(way_lenght_list))
                #     self.way_zuhe_best[zuhe] = way_lenght_list[a_1]
                #     self.way_data_best[zuhe] = copy.deepcopy(success_way_list[a_1])
                #     self.ways_data[zuhe].append(copy.deepcopy(success_way_list[a_1]))######储存四组不同权重系数组合的路线的列表
                ...
                if self.way_zuhe_best[zuhe]>min(zuhecost_list):
                    ...
                    self.ways_data[zuhe]=copy.deepcopy(success_way_list[a_1])

                    print('成功路径展示：',copy.deepcopy(success_way_list[a_1]),'节点个数：',zhandian_count[a_1])############
                ...
                self.each_iter_cost.append(avg_iter_cost_averge)   ###每代的成功搜索代价总值总和/历次迭代
                self.avg_cost_success_rate.append(avg_success_rate)   ###历次迭代的平均搜索成功率



                # Save variables to file
                with open('MIBSA_100ant.txt', 'a') as f:
                    ...
                    f.write(f"all_iter_cost: {self.each_iter_cost}\n")  
                ...
                print('每代平均长度:',np.average(way_lenght_list),'每代最少站点数:',np.min(zhandian_count))         ##############################################也没明白，也是要把数学形式列出来看看
